		...
		'columns'      => [
			...
			[
				...
				'value'     => function ($model) {
					return Html::a($model->order_code, Yii::$app->urlManager->createUrl([
						'order/update',
						'id' => $model->order_id,
					]));
				},
			...
			[
				...
				'format'    => 'raw',
				'value'     => function ($model) {
					return Html::a($model->customer_code, Yii::$app->urlManager->createUrl([
						'customer/update',
			...
			[
				...
				'format'    => 'raw',
				'value'     => function ($data) {
					$url = Yii::$app->urlManager->createUrl(['treatment-schedule/vote']);
					if($data->is_vote == 0) {
						return "<button class='btn btn-success' onclick='vote($data->id); return false;'>Đánh giá</button>";
					} else {
						return 'Đã/đang đánh giá';
					}
				},
			],
			[
