		...
		<div class="row">
			<div class="col-md-6">
				...
				<?php DynamicFormWidget::begin([
					'widgetContainer' => 'dynamicform_wrapper_service',
					// required: only alphanumeric characters plus "_" [A-Za-z0-9_]
					'widgetBody'      => '.container-items_service',
					// required: css class selector
					'widgetItem'      => '.item_service',
					// required: css class
					'limit'           => 4,
					// the maximum times, an element can be cloned (default 999)
					'min'             => 1,
					// 0 or 1 (default 1)
					'insertButton'    => '.add-item_service',
					// css class
					'deleteButton'    => '.remove-item_service',
					// css class
					...
					<?php foreach($modelServices as $index => $modelService): ?>
						<div class="item_service clearfix"><!-- widgetBody -->
							...
							<?= $form->field($modelService, "[{$index}]product_id")->widget(DepDrop::classname(), [
								'data'          => $districtList,
								'options'       => [
									'id'       => "order-{$index}-product_id",
									'prompt'   => 'Vui Lòng Chọn',
									"onchange" => "changeProduct(this.id)",
								],
								'pluginOptions' => [
							...
							<?= $form->field($modelService, "[{$index}]price", ['inputOptions' => ['value' => Yii::$app->formatter->asDecimal($model->price)]])->textInput([
								'maxlength' => true,
								'readonly'  => true,
							])->label("Đơn giá") ?>
			...
			<div class="container-items"><!-- widgetContainer -->
				<?php foreach($modelCheckouts as $index => $modelCheckout): ?>
					<div class="item clearfix"><!-- widgetBody -->
						...
						?>
						<div class="col-md-6">
							<?= $form->field($modelCheckout, "[{$index}]money")->textInput([
								'maxlength' => true,
								"onkeyup"   => "changeValue(this.value, this)",
							])->label("Lần thanh toán " . ($index + 1)) ?>
						</div>
						<div class="col-md-6">
							<?= $form->field($modelCheckout, "[{$index}]cash_type", ['template' => "<div class=\"col-lg-12\">{input}</div>"])->radioList([
								'1' => 'Tiền mặt',
								'2' => 'Thẻ',
							])->label(false) ?>
						</div>
