...
class TreatmentScheduleController extends Controller {
	...
	public function actionUpdate($id, $url = "") {
		...
		$query                      = Chat::find()->where(['treatment_schedule_id' => $id])->orderBy(['id' => SORT_ASC])->all();
	...
	public function actionChat($id) {
		...
		if(Yii::$app->request->post('message')) {
			...
			$chat->username              = User::findOne(Yii::$app->user->id)->username;
			$chat->treatment_schedule_id = $id;
			$chat->customer_id           = $model->customer_id;
