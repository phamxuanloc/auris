<?php
namespace app\components\actions\filekit;
    ...
    public function run()
    {
        ...
        foreach ($uploadedFiles as $uploadedFile) {
            ...
            $output = [
                ...
                $this->responseBaseUrlParam =>  Yii::$app->request->hostInfo.'/uploads',
            ];

            if ($uploadedFile->error === UPLOAD_ERR_OK) {
                $validationModel = DynamicModel::validateData(['file' => $uploadedFile], $this->validationRules);
                if (!$validationModel->hasErrors()) {
                    $path = $this->getFileStorage()->save($uploadedFile, false, false, (array)FileHelper::getFileInfo($uploadedFile->name));
                    if ($path) {
                        $output[$this->responsePathParam] = $path;
                        $output[$this->responseUrlParam] = Yii::$app->params['urlPath'] . '/' . $path;
